...
namespace MoRS.ManagementSystem.Infrastructure.Services
{
    public class UserManagementService
    {
        ...
        public async Task<User?> UpdateUserAsync(int userId, UpdateUserRequest request)
        {
            ...
            var existingUserWithPhone = await _userRepository.GetAsync(new MoRS.ManagementSystem.Application.Filters.UserQuery());
            var phoneExists = existingUserWithPhone.Any(u => u.Id != userId && u.PhoneNumber == request.PhoneNumber);
            if (phoneExists)
            {
